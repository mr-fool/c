...
//since text editor can't handle null character I am force to open it manually
void cmdline(int id) {
	sprintf(filename, "/proc/%d/cmdline", pid);
	file = fopen(filename, "r");
	int size;
	char buf[512];
	while(0 < (size = fread(buf, 1, 512, file))){
		int i;
		for(i = 0; i < size; i++) {
			if(buf[i] == 0) {
				buf[i] = ' ';
...
/*
./ps457.out -v -s -c -S -U -p 1
PID	state	utime	stime	vmem	cmdline	
1	S	76	227	47074	/usr/lib/systemd/systemd --switched-root --system --deserialize 21 
[mr-fool@localhost HW]$ ./ps457.out -v -s -c -S -U
PID	state	utime	stime	vmem	cmdline	
